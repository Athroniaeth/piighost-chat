...
export function DialogContent({
  ...
  return (
    <Modal
      {...modalProps}
      className={cn(
        "fixed top-1/2 left-1/2 z-50 w-[calc(100%-32px)] sm:w-full max-w-lg -translate-x-1/2 -translate-y-1/2 outline-none transition-all duration-200 data-entering:scale-95 data-entering:opacity-0 data-exiting:scale-95 data-exiting:opacity-0",
        modalProps?.className,
      )}
    >
      <AriaDialog
        className={cn(
          "w-full relative rounded-xl border border-base-100 p-5 bg-background-100 shadow-lg outline-none",
          className,
        )}
        {...props}
      >
